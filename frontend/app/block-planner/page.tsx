...
export default function BlockPlanner() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold mb-1">Block Planner</h1>
      <p className="text-sm text-gray-500 mb-6">Select compatible maintenance tasks, then find the lowest-impact common block window.</p>

      <div className="bg-white rounded-xl shadow p-5 mb-6">
        <div className="flex flex-wrap gap-4 mb-4">
          <label className="text-sm">Section
            <select value={section} onChange={(e) => setSection(e.target.value)} className="ml-2 border rounded-lg px-3 py-1.5 text-sm">
          ...
          <label className="text-sm">Date
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="ml-2 border rounded-lg px-3 py-1.5 text-sm" />
          </label>
          <button onClick={find} disabled={loading} className="ml-auto px-5 py-2 rounded-lg bg-teal-600 text-white text-sm font-medium hover:bg-teal-700 disabled:opacity-50">
      ...
      {error && <p className="mb-4 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2">{error}</p>}

      {compat && (
        <div className="bg-white rounded-xl shadow p-5 mb-6 text-sm text-gray-600">
          <span className="font-semibold text-gray-800">Compatibility: </span>{compat.summary}
      ...
      {rec && (
        <div className="bg-teal-50 border border-teal-200 rounded-xl p-5 mb-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-teal-600 text-white">{rec.confidence}</span>
          </div>
          <p className="text-sm text-gray-700">{rec.reason}</p>
        </div>
      )}

      <h2 className="font-semibold mb-3">Candidate Windows</h2>
